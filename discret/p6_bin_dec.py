...
def create_pow(color=1, pow=0):
    return VGroup(
        ...
        MathTex("1000" + (f"^{pow}" if pow != 1 else "")).scale(1.2)
        if pow != 0
        else Text("Byte"),
    )
...
class P6BinDec(Scene):
    def construct(self):
        ...
        VGroup(bytes_dec, bytes_bin).arrange(RIGHT, buff=2)
        ...
        for o in bytes_dec[2:]:
            ...
            self.play(TransformFromCopy(o[0][0], o[2], path_arc=-PI / 2))
            self.wait(0.5)
        ...
        self.play(TransformFromCopy(bytes_dec[2:], bytes_bin[2:]))
        ...
        eq = (
            MathTex(r"\frac{1000^3}{1024^3}", "=", f"{1000**3 / 1024**3:.2f}")
            .scale(2)
            .to_edge(DOWN)
        )
        ...
        self.play(TransformFromCopy(eq[0], eq[-1], path_arc=-PI / 2))

        self.wait()
        eqc = MathTex("0.93").scale(2).to_edge(DOWN)
        self.play(
            # FadeOut(eq[:-1]),
            TransformMatchingTex(eq, eqc)
        )
        ...
        self.play(ReplacementTransform(eqc, examp[2][:4]))
        self.wait()
        self.play(
            Write(examp[2][4]),
            TransformFromCopy(examp[0], examp[2][5:], path_arc=-PI / 2),
        )

        self.wait()

        self.play(Write(examp[3]))
        self.play(TransformFromCopy(examp[2], examp[4], path_arc=-PI / 2))
        ...
        self.wait()
        examp2 = MathTex(r"1 \text{ TB}", "=", r"1000 \text{ GB}").scale(3).shift(DOWN)
    ...
    def pow(self, unity: VGroup, u_title: Text, u_name: str, pow=0):
        ...
        brace_text = MathTex("1000").scale(1.5).next_to(brace, DOWN)
        ...
        value = (
            MathTex(("1000" + f"^{pow}") if pow > 0 else "1", r"\cdot", "1000")
            .scale(2)
            .to_edge(DOWN)
        )
        ...
        self.play(
            Transform(value, MathTex(f"{1000 ** (pow + 1)}").scale(2).to_edge(DOWN))
        )
