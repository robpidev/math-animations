...
class P8PixelBit(Scene):
    def construct(self):
        ...
        for s, v in zip(pixel[1], pixel[2]):
            self.play(Create(s), Write(v), run_time=0.5)
        ...
        self.play(*[value_trackers[i].animate.set_value(0) for i in range(3)])

        self.wait()

        self.play(*[value_trackers[i].animate.set_value(255) for i in range(3)])
        ...
        self.play(TransformFromCopy(VGroup(pixel[1][0], pixel[2][0]), red_byte))
        ...
        self.play(TransformFromCopy(VGroup(pixel[1][1], pixel[2][1]), green_byte))
        ...
        self.play(TransformFromCopy(VGroup(pixel[1][2], pixel[2][2]), blue_byte))
        ...
        self.play(Transform(bc_text, bc.get_tex(r"\text{8 bits (b)}").scale(1.5)))

        self.wait()

        self.play(Transform(bc_text, bc.get_tex(r"\text{Byte (B)}").scale(1.5)))
