...
def pixel_with_updaters(red=0, green=0, blue=0):
    ...
    sq = RoundedRectangle(height=5, width=5, corner_radius=0.1)
    ...
    r.add_updater(lambda m: m.set_fill(color=rgb(rvt.get_value(), 0, 0), opacity=1))

    g.add_updater(lambda m: m.set_fill(color=rgb(0, gvt.get_value(), 0), opacity=1))

    b.add_updater(lambda m: m.set_fill(color=rgb(0, 0, bvt.get_value()), opacity=1))
    ...
    # Text updater
    rt.add_updater(
        lambda m: m.become(Text(f"{int(rvt.get_value())}", font=f).move_to(r))
    )

    gt.add_updater(
        lambda m: m.become(Text(f"{int(gvt.get_value())}", font=f).move_to(g))
    )

    bt.add_updater(
        lambda m: m.become(Text(f"{int(bvt.get_value())}", font=f).move_to(b))
    )
...
class P7Pixel(Scene):
    def construct(self):
        ...
        pixel = RoundedRectangle(height=4, width=4, corner_radius=0.1).set_fill(
        ...
        pixel.add_updater(
            lambda m: m.set_fill(
                color=rgb(rvt.get_value(), gvt.get_value(), bvt.get_value()), opacity=1
            )
        )

        self.play(TransformFromCopy(VGroup(r, g, b), pixel))
        ...
        pixels = [[rgb(119, 117, 205) for _ in range(16)] for _ in range(9)]
        ...
        img_white = [[rgb(255, 255, 255) for _ in range(16)] for _ in range(9)]

        self.play(Transform(img_ob, create_imgob(img_white).scale(0.3)))

        self.wait()

        img_grad = [
            [rgb(2 * i * j, 2 * i * j, 2 * i * j) for i in range(16)] for j in range(9)
        ]

        self.play(Transform(img_ob, create_imgob(img_grad).scale(0.3)))
        ...
        self.play(Transform(img_ob, create_imgob(img_rand).scale(0.3)))
        ...
        self.play(Transform(img_ob, create_imgob(img_16x16).scale(0.2)), FadeOut(res))
        ...
        self.play(Transform(img_ob, create_imgob(img_heart).scale(0.2)))
        ...
        inf = MathTex("16", "\\times", "16", "\\times", "3", r"\text{ Bytes}")
        inf.scale(1.5).to_edge(DOWN)

        # self.next_section(skip_animations=False)

        self.play(
            TransformMatchingShapes(VGroup(hb_text, wb_text), inf[:3]), FadeOut(hb, wb)
        )
