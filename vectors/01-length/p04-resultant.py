...
class P04_Resultant(Scene):
    def construct(self):
        ...
        title = Tex(
            "Longitud de la resultante: $\|\mathbf{u + v}\|$"
        )
        ...
        uvl, vvl = MathTex(r"\mathbf{u}", color=RED), MathTex(r"\mathbf{v}", color=GREEN)
        ...
        t1t = MathTex(r"\alpha", color=RED).move_to(
            Angle(xL, yL,
                radius=0.8 + 3 * SMALL_BUFF).point_from_proportion(0.5)
        )
        ...
        t2t = MathTex(r"\beta", color=GREEN).move_to(
            Angle(xL, yL,
                radius=0.7 + 3 * SMALL_BUFF).point_from_proportion(0.5)
        )
        ...
        thetal = MathTex(r"\theta").move_to(
            Angle(uL, vL,
                radius=1 + 3 * SMALL_BUFF).point_from_proportion(0.5)
        )
        ...
        rtex = MathTex(
            r"\mathbf{u + v}", "=",
            vec_matrix(r"u\cos(\alpha) + v\cos(\beta)", r"u\sin(\alpha) + v\sin(\beta)"),
        )
        ...
        self.play(
            ReplacementTransform(
                Group(uvtex[0], vvtex[0]), rtex[0]
            ),
            FadeIn(rtex[1], rtex[2][0], rtex[2][-1]),
            TransformMatchingShapes(
                Group(uvtex[2][1:8], vvtex[2][1:8]), rtex[2][1:16]
            ),
            TransformMatchingShapes(
                Group(uvtex[2][8:-1], vvtex[2][8:-1]), rtex[2][16:-1]
            ),
            FadeOut(uvtex[2][0], uvtex[2][-1], vvtex[2][0], vvtex[2][-1],
                    uvtex[1], vvtex[1]),
        )
        self.wait()
        ...
        ).scale(0.6).move_to(mod_res)

        self.play(
            ReplacementTransform(
                rtex[0].copy(), mod_res[0][1:-1]
            ),
            FadeIn(mod_res[0][0], mod_res[0][-1], mod_res[1]),
            Write(VGroup(
                mod_res[2][:3], mod_res[2][18:22],
                mod_res[2][-2:]
            )),
            ReplacementTransform(
                rtex[2][1:16].copy(), mod_res[2][3:18]
            ),
            ReplacementTransform(
                rtex[2][16:-1].copy(), mod_res[2][22:-2]
            )
        )
