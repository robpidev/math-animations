...
class Vec_Set(Scene):
    def construct(self):
        ...
        set_r = MathTex(
            r"\mathbb{R}^2",
            r"=\mathbb{R} \times \mathbb{R}",
            r"=\left\{(x, y):x,y\in\mathbb{R}\right\}",
        )
        ...
        esc = Tex(
            r"Se acostumbra escribir\\", "$(x + y) + z = z + y + z$, $x + (-y) = x - y$"
        )

        vfield = VGroup(field_r, ad1, ad2, ad3, ad4, ad5, esc).arrange(
            DOWN, aligned_edge=LEFT
        )

        self.play(TransformMatchingShapes(vec_r[0], field_r[2]), FadeOut(vec_r[1:]))
