...
class Add3(Scene):
    def construct(self):
        ...
        uvl = MathTex(r"\mathbf{u}", "+", r"\mathbf{v}", color=YELLOW).move_to(
            [-0.5, 0, 0]
        )
        vwl = MathTex(r"\mathbf{v}", "+", r"\mathbf{w}", color=ORANGE).move_to(
            [0, 0.5, 0]
        )
        ruvl = MathTex(
        ...
        eq1 = MathTex(
            ...
            vec(vec_ab(pr, ps)),
            "=",
            vec(vec_add(vec_ab(pp, pq), vec_ab(pq, pr))),
            "+",
            vec(vec_ab(pr, ps)),
            "=",
        ...
        self.play(
            vecs1.animate.scale(0.5).move_to([5, 1.7, 0]),
        ...
        eq2 = MathTex(
            ...
            vec(vec_ab(pp, pq)),
            "+",
            vec(vec_add(vec_ab(pq, pr), vec_ab(pr, ps))),
            "=",
            vec(vec_ab(pp, ps)),
        ).move_to([0, -3, 0])
        ...
        self.play(
            vecs2.animate.scale(0.5).move_to([5, -2.2, 0]),
            FadeOut(eq2, rvwc),
            vwg.animate.set_opacity(1),
        )

        self.wait()
        r = Arrow(pp, ps, color=RED, buff=0)
        rl = MathTex(r"\mathbf{u} + \mathbf{v} + \mathbf{w}", color=RED).move_to(
            [0, -2.2, 0]
        )
        ...
        l2 = MathTex(
            ...
            "=",
            vec_matrix(
                r"\left(u_x + v_x\right) + w_x", r"\left(u_y + v_y\right) + w_y"
            ),
        )
        ...
        self.play(TransformFromCopy(l4[6], l4[14]), TransformFromCopy(l4[10], l4[18]))
        ...
        self.play(
            l1[0:4].animate.move_to(result[1:3].get_center()).scale(1),
        ...
        self.play(
            FadeOut(res_g[:-1], res_g[-1][1:], vecs1, vecs2, a3),
        )
