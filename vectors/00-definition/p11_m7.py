...
class Prop7(Scene):
    def construct(self):
        ...
        rl2 = MathTex(r"2", r"(\mathbf{u} + \mathbf{v})", color=RED).move_to(r2, UP)
        ...
        ul2 = MathTex(r"2", r"\mathbf{u}", color=GREEN_B).move_to(u2, DOWN)
        self.play(
            Create(u2),
        ...
        v2 = Arrow(vec_scale(2, a), vec_scale(2, b), buff=0, color=BLUE_B).set_opacity(
            0.8
        )
        vl2 = MathTex(r"2", r"\mathbf{v}", color=BLUE_B).move_to(v2, LEFT)
        self.play(
        ...
        self.play(TransformMatchingShapes(Group(ul2.copy(), vl2.copy()), rl2_add))

        self.wait()
        graf = Group(vecs_org, u2, v2, ul2, vl2, r2, rl2, rl2_add)

        self.play(graf.animate.scale(0.7).move_to([4.5, -2.5, 0]), FadeOut(plane))
        ...
        l1 = MathTex(
            r"c\in\mathbb{R}",
            r"\wedge" + r"\mathbf{u}, \mathbf{v}\in \mathbb{R}^2",
            ",",
        )
        ...
        l5 = MathTex(
            r"c", r"(\mathbf{u} + \mathbf{v})", "=", r"c\mathbf{u}", "+", r"c\mathbf{v}"
        )
        ...
        self.play(
            TransformMatchingShapes(VGroup(l3[3][1:4], l3[3][8:11]).copy(), l3[5]),
            TransformFromCopy(VGroup(l3[3][4], l3[3][11]), l3[6]),
            TransformMatchingShapes(VGroup(l3[3][5:8], l3[3][12:15]).copy(), l3[7]),
        )
        self.wait()
