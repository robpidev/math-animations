...
class Prop8(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(u2.animate.move_to([-2, -3, 0]), u2l.animate.move_to([-2.5, -2.5, 0]))
        self.wait()
        self.play(Create(u3), Write(u3l))
        self.wait()
        self.play(u3.animate.move_to([3, -0.5, 0]), u3l.animate.move_to([2.4, -0.3, 0]))
        self.wait()

        r = Arrow([-4, -4, 0], [6, 1, 0], buff=0, color=RED).set_z_index(6)
        r.set_opacity(0.6)
        rl = MathTex(
            "5", r"\mathbf{u}", "=", r"2\mathbf{u} + 3\mathbf{u}", color=RED
        ).move_to([2.5, -2.5, 0])

        rl_alt = MathTex(
            "(2 + 3)", r"\mathbf{u}", "=", r"2\mathbf{u} + 3\mathbf{u}", color=RED
        ).move_to([2.5, -2.5, 0])

        self.play(TransformFromCopy(u, r), TransformMatchingShapes(ul.copy(), rl[0:2]))

        self.wait()
        self.play(TransformMatchingShapes(Group(u2l, u3l).copy(), rl[2:]))
        self.wait()
        ...
        m2 = (
            MathTex(r"\text{(M2). }xy = yx, \quad\forall x, y \in \mathbb{R}")
            .scale(0.8)
            .to_edge(UP + LEFT)
            .shift(0.5 * DOWN)
        )
        ...
        self.play(
            TransformMatchingShapes(Group(l2[5][1:4], l2[5][8:11]).copy(), l3[2]),
            TransformFromCopy(VGroup(l2[5][4], l2[5][11]), l3[3]),
            TransformMatchingShapes(Group(l2[5][5:8], l2[5][12:15]).copy(), l3[4]),
        )
        ...
        self.play(
            TransformMatchingShapes(l1, result[6:], fade_transform_mismatches=True)
        )

        self.wait()
        self.play(Write(result[0]))

        self.play(result.animate.scale(0.8).to_edge(UP + LEFT), FadeOut(d_alt[1:], fig))
        self.wait()
        ...
        self.play(
            TransformMatchingShapes(
                eq[0].copy(), eq[2], transform_mismatches=True, path_arc=PI / 2
            )
        )
        ...
        self.play(
            Transform(Group(eq1[2][0], eq1[2][3]).copy(), eq1[4][0]),
            TransformMatchingShapes(
                Group(eq1[2][1:3], eq1[2][-1]).copy(),
                eq1[4][1:],
